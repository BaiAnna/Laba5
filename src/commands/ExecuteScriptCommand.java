        ...
        try {
            ...
            while (scanner.hasNext()) {
                ...
                if (line.equals("help")) {
                    ...
                    System.out.println(executor.executeUpdateIdElement(readIdElement("update", line),readName(scanner), readCoordinatesX(scanner), readCoordinatesY(scanner), readFromLocationX(scanner), readFromLocationY(scanner), readFromLocationZ(scanner), readFromLocationName(scanner), readToLocationX(scanner), readToLocationY(scanner), readToLocationZ(scanner), readDistance(scanner) ));
                } else if(line.contains("remove_by_id")){
                    System.out.println(executor.executeRemoveById(readIdElement( "remove_by_id", line)));
            ...
            return e.getMessage()+" "+"Скрипт содержит ошибки, поэтому его выполнение приостановлено!";
        }catch (FileNotFoundException e){
            return "Такого файла не найдена";
        }catch(Exception e){
            return "Что-то пошло не так ";
        }
        return "Выполнение команды закончено";
    }
